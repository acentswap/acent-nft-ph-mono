...
import {CoinGeckoAPIForBNB, CoinGeckoAPIForAcent} from './constants';
...
const App = () => {
  ...
  useEffect(() => {
    let interval = setInterval(async () => {
      ...
      const resACENT = await fetch(CoinGeckoAPIForAcent);
      const returnedBNB = await resBNB.json();
      const returnedAcent = await resACENT.json();
      dispatch(
        updatePrice({
          bnb: returnedBNB.binancecoin.usd,
          acent: returnedAcent.acent.usd,
        })
      );
    }, 60000);
